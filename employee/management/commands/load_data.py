...
class Command(BaseCommand):
    ...
    def handle(self, *args: Any, **options: Any):
        ...
        nlaurl = str("http://paymastergh.com:8049/NLA/ODataV4/Company('NLA%20JUN%20AND%20SEN')/EmployeeCard")
        payroll_url = str("http://paymastergh.com:7048/Payroll/ODataV4/Company('RCH%20HOD')/EmployeeCard")
        emery = str("http://paymastergh.com:7048/Payroll/ODataV4/Company('Emery%20Invest')/EmployeeCard")
        payroll_res = requests.get(url=payroll_url, auth=auth)
        nlares = requests.get(url=nlaurl, auth=auth)

        print(nlares.status_code)
        print(payroll_res.status_code)
        payroll_data = payroll_res.json()
        nla_data = nlares.json()
        data = [payroll_data, nla_data]
        pprint(len(data[0]))

        get_user_data(url=emery, auth=auth, company='Emery')

        # pprint(data)
        # for employee in nla_data['value']:
